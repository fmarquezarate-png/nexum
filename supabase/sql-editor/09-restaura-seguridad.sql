-- ═══════════════════════════════════════════════════════════════════════
--  NEXUM — ACTUALIZACIÓN 09: devuelve la seguridad a la tabla de casas
-- ═══════════════════════════════════════════════════════════════════════
--
--  ⚠ PÉGALO CUANTO ANTES ⚠
--
--  Al depurar el fallo de "crear casa" se desactivó a mano la protección
--  de la tabla homes. Funcionó, pero dejó la tabla abierta: la clave
--  pública viaja dentro de la app, así que cualquiera que abra la web
--  podría leer, crear o borrar casas.
--
--  Este archivo la devuelve a su sitio y, de paso, revisa que no se haya
--  quedado ninguna otra tabla sin proteger.
--
--  No pierdes nada: crear casas sigue funcionando gracias a la función
--  create_home que añadió la actualización 08.
--
--  ⚠ ANTES DE PEGAR: VACÍA EL EDITOR ⚠
--  Pulsa dentro del recuadro, selecciona todo (Ctrl+A / Cmd+A) y bórralo,
--  o abre una consulta nueva con "New query".
--
--  CÓMO SE USA
...
-- ▼▼▼ 20260915090000_restaura_rls_homes.sql ▼▼▼
-- ═══════════════════════════════════════════════════════════════════════
...
insert into supabase_migrations.schema_migrations (version, name) values
  ('20260915090000', 'restaura_rls_homes')
on conflict (version) do nothing;

commit;

-- ═══════════════════════════════════════════════════════════════════════
--  COMPROBACIÓN — ejecuta esto DESPUÉS, en una consulta NUEVA
-- ═══════════════════════════════════════════════════════════════════════
--  Deben salir 16 tablas y TODAS con protegida = true.
--
--  select tablename, rowsecurity as protegida
--  from pg_tables where schemaname = 'public' order by protegida, tablename;
